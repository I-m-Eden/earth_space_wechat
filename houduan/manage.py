...
def openid():
    db = pymysql.connect(host='127.0.0.1', user='root',
                         password=os.environ.get('MYSQL_PASSWORD'), db='demo')
    cursor = db.cursor()
    data = request.json
    print(data)
    redata = {}
    ...
    url = 'https://api.weixin.qq.com/sns/jscode2session?' + \
        'appid=wx89123eb2d88625c8&' + \
        'secret=6ea050b15e40fa697540cb7aeff5b079&' + \
        'js_code=' + data['code'] + \
...
def getfreq():
    db = pymysql.connect(host='www.pkusess.asia', user='root',
                         password=os.environ.get('MYSQL_PASSWORD'), db='demo')
    cursor = db.cursor()
    data = request.json
    redata = {}
    ...
    if cursor.execute("SELECT freq FROM students WHERE openid = '%s'" % (data['openID'])) != 0:
        ...
        redata['last'] = freqall - freq[0][0]
    db.close()
    redata['nexttime'] = '您的本轮挑战次数已达上限或本轮挑战已截止，下轮开始时间：暂定6月1日'
    return json.dumps(redata, ensure_ascii=False)
...
def create():
    ...
    redata = {}
    redata = data['value']
    sql2 = "INSERT INTO questions(title, opa, opb, opc, opd, opr)\
                values('%s', '%s', '%s', '%s', '%s', '%s')"\
                        % (redata['title'], redata['opa'], redata['opb'],
                           redata['opc'], redata['opd'], '.'+redata['opr']+'.'+data['openID'])
    try:
        print(sql2)
        cursor.execute(sql2)
        db.commit()
...
def questionget():
    ...
    if cursor.execute("SELECT did FROM students WHERE openid = '%s'" % (data['openID'])) != 0:
        ...
        qfreq = cursor.fetchall()
        flag = 1
    if qfreq[0][0] > 40000000:
       print('有人作弊')
       redata['title'] = '您的答题次数已达到上限'
       return json.dumps(redata, ensure_ascii=False)
    else:
        if did[0][0] == '0':
...
@app.route('/tips1', methods=['GET'])
def tips1():
    md_data = open(os.path.dirname(os.path.realpath(
        __file__)) + '/tips/tip1.md', "rb").read()
    response = make_response(md_data)
    response.headers['Content-Type'] = 'text/md'
    return response

@app.route('/tips2', methods=['GET'])
def tips2():
    md_data = open(os.path.dirname(os.path.realpath(
        __file__)) + '/tips/tip2.md', "rb").read()
    response = make_response(md_data)
    response.headers['Content-Type'] = 'text/md'
    return response

@app.route('/tips3', methods=['GET'])
def tips3():
    md_data = open(os.path.dirname(os.path.realpath(
        __file__)) + '/tips/tip3.md', "rb").read()
    response = make_response(md_data)
    response.headers['Content-Type'] = 'text/md'
    return response

@app.route('/tips4', methods=['GET'])
def tips4():
    md_data = open(os.path.dirname(os.path.realpath(
        __file__)) + '/tips/tip4.md', "rb").read()
    response = make_response(md_data)
    response.headers['Content-Type'] = 'text/md'
    return response
